...
async def create_order(
    ...
    # products so each maps to a real SpaceSeller SKU, carrying the pack's quantity and
    # that constituent's real per-tier unit price (e.g. 2-pack → velva 2×499, colla 2×569).
    marketplace_items = []
    for l in lines:
        for s in _pack_slugs(l["slug"]):
