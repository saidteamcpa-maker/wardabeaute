...
export async function WardaPage({ slug, preview = false }: { slug: string; preview?: boolean }) {
  ...
  const offerImages = c.pricing.cards.map((_, index) =>
    (ov?.[`${imageKey === "kit" ? "kit" : "pp"}.offer${index + 1}Image`] as string) || heroImage,
  );
  ...
  return (
    <div dir={dir} className={lang === "ar" ? "font-arabic" : ""}>
      ...
      <section dir={dir} className="pt-12 md:pt-16 lg:pt-20 pb-2 md:pb-3 bg-petal/30 scroll-mt-6">
        <div className="container-page">
          ...
          <p className="font-body text-gris text-xs text-center mt-2 leading-relaxed">{c.pricing.trust}</p>
        </div>
      </section>
